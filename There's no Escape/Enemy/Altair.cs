...
public class Altair : MonoBehaviour
{
    ...
    void Follow()
    {
        ...
        if (right == true && bite == false)
        {
            ...
            transform.localScale = new Vector3(0.30409f, 0.30409f, 0.30409f);
        }
    ...
    void Patrol()
    {
        ...
        if (right == true)
        {
            Debug.DrawLine(new Vector3(transform.position.x + 2, transform.position.y - 1.4f, transform.position.z), new Vector3(transform.position.x + 1.5f, transform.position.y - 1.4f, transform.position.z), Color.red);
            RaycastHit2D groundCheck = Physics2D.Raycast(new Vector3(transform.position.x + 2, transform.position.y - 1.4f, transform.position.z), -Vector2.right, 0.5f);
        ...
        if (right == true) { control.velocity = new Vector3(2f, control.velocity.y); transform.localScale = new Vector3(0.30409f, 0.30409f, 0.30409f); }
        if (right == true) { control.velocity = new Vector3(2f, control.velocity.y); transform.localScale = new Vector3(0.30409f, 0.30409f, 0.30409f); }
        else { control.velocity = new Vector3(-1.5f, control.velocity.y); transform.localScale = new Vector3(-0.30409f, 0.30409f, 0.30409f); }

        if (turn == true)
        {
            if (right == true) { right = false; } else { right = true; }
            if (right == true) { transform.localScale = new Vector3(0.30409f, 0.30409f, 0.30409f); }
            else { transform.localScale = new Vector3(-0.30409f, 0.30409f, 0.30409f); }
            turn = false;
        }
    }
